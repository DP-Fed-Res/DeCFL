...
import torch
from customopacus.optimizers.utils import params
...
def _check_processed_flag_tensor(x: torch.Tensor):
    ...
    See Also:
        :meth:`~customopacus.optimizers.optimizer._mark_as_processed`
...
def _generate_noise(
    ...
    Notes:
        ...
        The attack for customopacus first appeared in https://arxiv.org/abs/2112.05307.
...
class DPOptimizer(Optimizer):
    ...
    def signal_skip_step(self, do_skip=True):
        ...
        Used by :class:`~customopacus.utils.batch_memory_manager.BatchMemoryManager` to
    ...
    def add_noise(self):
        ...
        if self.Vk is None:
            ...
            summed_grad = torch.cat([p.summed_grad.view(-1) for p in self.params])
            p_summed_grad = torch.matmul(summed_grad, self.Vk)
            p_noise = _generate_noise(
                std=self.noise_multiplier * self.max_grad_norm,
                reference=p_summed_grad,
                generator=self.generator,
                secure_mode=self.secure_mode,
            )
            rec_grad = torch.matmul(p_summed_grad + p_noise, self.Vk.T)

            start_idx = 0
            for p in self.params:
                _check_processed_flag(p.summed_grad)
                end_idx = start_idx + len(p.view(-1))
                p.grad = rec_grad[start_idx:end_idx].reshape(p.shape).view_as(p)
                start_idx = end_idx
                _mark_as_processed(p.summed_grad)
    ...
    def zero_grad(self, set_to_none: bool = False):
        ...
        if set_to_none is False:
            logger.debug(
                ...
                "customopacus will set p.grad_sample and p.summed_grad to None due to "
