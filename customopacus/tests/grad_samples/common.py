...
from customopacus.utils.per_sample_gradients_utils import (
    compute_grad_samples_microbatch_and_customopacus,
    compute_customopacus_grad_sample,
    is_batch_empty,
)
...
class GradSampleHooks_test(unittest.TestCase):
    ...
    def run_test_with_reduction(
        ...
        if not type(x) is PackedSequence and is_batch_empty(x):
            _ = compute_customopacus_grad_sample(
                x,
                module,
                batch_first=batch_first,
                loss_reduction=loss_reduction,
            ...
            # We've checked customopacus can handle 0-sized batch. Microbatch doesn't make sense
            return
        (
        ...
        self.check_shapes(microbatch_grad_samples, customopacus_grad_samples, loss_reduction)
        self.check_values(
            microbatch_grad_samples, customopacus_grad_samples, loss_reduction, atol, rtol
        )
    ...
    def check_shapes(
        ...
        for name, customopacus_grad_sample in customopacus_grad_samples.items():
            ...
            msg = (
                f"Param '{name}': "
                f"from customopacus: {customopacus_grad_sample.shape}, "
                f"from Microbatch: {microbatch_grad_sample.shape}. "
            )
            try:
                self.assertEqual(
                    customopacus_grad_sample.shape,
    ...
    def check_values(
        ...
        for name, customopacus_grad_sample in customopacus_grad_samples.items():
            microbatch_grad_sample = microbatch_grad_samples[name]
            msg = (
                f"Param {name}: customopacus L2 norm = : {customopacus_grad_sample.norm(2)}, ",
                f"Microbatch L2 norm = : {microbatch_grad_sample.norm(2)}, ",
                f"MSE = {F.mse_loss(customopacus_grad_sample, microbatch_grad_sample)}, ",
                f"L1 Loss = {F.l1_loss(customopacus_grad_sample, microbatch_grad_sample)}",
            )
            try:
                assert_close(
                    actual=microbatch_grad_sample,
                    expected=customopacus_grad_sample,
                    atol=atol,
                    rtol=rtol,
                )
            except AssertionError:
