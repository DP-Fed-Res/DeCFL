...
from customopacus.grad_sample import GradSampleModule, GradSampleModuleFastGradientClipping
from customopacus.optimizers import DPOptimizer, DPOptimizerFastGradientClipping
from customopacus.utils.fast_gradient_clipping_utils import DPLossFastGradientClipping
from customopacus.utils.per_sample_gradients_utils import clone_module
...
class GradSampleModuleFastGradientClippingTest(GradSampleModuleTest):
    ...
    def test_norm_calculation_fast_gradient_clipping(self, size, length, dim):
        """
        Tests if norm calculation is the same between standard (customopacus) and fast gradient clipping"
        """
        self.length = length
        self.size = size
        self.dim = dim
    ...
    def test_gradient_calculation_fast_gradient_clipping(self, size, length, dim):
        """
        Tests if gradients are the same between standard (customopacus) and fast gradient clipping"
        """
...
class GradSampleModuleFastGradientClippingEmbeddingLayerTest(unittest.TestCase):
    ...
    def test_norm_calculation(self):
        ...
        standard (customopacus) and fast gradient clipping"
        """
        vocab_size = 3
        embedding_dim = 1
    ...
    def test_gradient_calculation(self):
        ...
        (customopacus) and fast gradient clipping."""
