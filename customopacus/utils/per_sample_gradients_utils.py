...
from customopacus.grad_sample import wrap_model
from customopacus.utils.module_utils import trainable_parameters
from customopacus.utils.packed_sequences import compute_seq_lengths
    ...
    for _, p in trainable_parameters(module):
        if batch_first:
            ...
            p.microbatch_grad_sample = torch.stack(
                ...
                0, 1
            )  # customopacus's semantics is that grad_samples are ALWAYS batch_first: [B, T, ...]
...
def compute_customopacus_grad_sample(
    ...
    """
    Runs customopacus to compute per-sample gradients and return them for testing purposes.
...
def check_per_sample_gradients_are_correct(
    ...
    *,
    ...
    with the result of optimized customopacus method.
    ...
    Example:
        ...
        ...            x,
        ...            model
        ...        ) # This will fail only if the customopacus per sample gradients do not match the micro-batch gradients.
    """
    reductions = ["sum", "mean"]
...
def compute_grad_samples_microbatch_and_customopacus(
    x: Union[torch.Tensor, PackedSequence],
    module: nn.Module,
    batch_first: bool = True,
    loss_reduction: str = "mean",
    ...
    customopacus_grad_samples = compute_customopacus_grad_sample(
        x,
        module,
        batch_first=batch_first,
        loss_reduction=loss_reduction,
    ...
    if microbatch_grad_samples.keys() != customopacus_grad_samples.keys():
        raise ValueError(
            "Keys not matching! "
            f"Keys only in microbatch: {microbatch_grad_samples.keys() - customopacus_grad_samples.keys()}; "
            f"Keys only in customopacus: {customopacus_grad_samples.keys() - microbatch_grad_samples.keys()}"
        )

    return microbatch_grad_samples, customopacus_grad_samples
...
def _check_per_sample_gradients_are_correct_with_reduction(
    ...
    (
        microbatch_grad_samples,
        customopacus_grad_samples,
    ) = compute_grad_samples_microbatch_and_customopacus(
        x,
        module,
        batch_first=batch_first,
        loss_reduction=loss_reduction,
    ...
    for name, customopacus_grad_sample in customopacus_grad_samples.items():
        microbatch_grad_sample = microbatch_grad_samples[name]
        if not customopacus_grad_sample.shape == microbatch_grad_sample.shape:
            return False
        if not torch.allclose(microbatch_grad_sample, customopacus_grad_sample, atol, rtol):
            return False
    return True
