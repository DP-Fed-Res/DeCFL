...
customopacus_PARAM_MONKEYPATCH_ATTRS = ["_forward_counter", "_current_grad_sample"]
...
class AbstractGradSampleModule(nn.Module, ABC):
    ...
    def zero_grad(self, set_to_none: bool = False):
        ...
        if set_to_none is False:
            logger.debug(
                ...
                "customopacus will set p.grad_sample to None due to "
    ...
    def _clean_up_attributes(self):
        for attr in customopacus_PARAM_MONKEYPATCH_ATTRS:
            for p in self.parameters():
                if hasattr(p, attr):
                    delattr(p, attr)
