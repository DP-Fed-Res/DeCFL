...
import torch
from customopacus.accountants import create_accountant
from customopacus.accountants.utils import get_noise_multiplier
from customopacus.data_loader import DPDataLoader, switch_generator
from customopacus.distributed import DifferentiallyPrivateDistributedDataParallel as DPDDP
from customopacus.grad_sample import (
    AbstractGradSampleModule,
    GradSampleModule,
    get_gsm_class,
    wrap_model,
)
from customopacus.optimizers import DPOptimizer, get_optimizer_class
from customopacus.schedulers import _GradClipScheduler, _NoiseScheduler
from customopacus.utils.fast_gradient_clipping_utils import DPLossFastGradientClipping
from customopacus.validators.module_validator import ModuleValidator
...
class PrivacyEngine:
    """
    Main entry point to the customopacus API - use ``PrivacyEngine``  to enable differential
    ...
    def __init__(self, *, accountant: str = "prv", secure_mode: bool = False):
        ...
        Args:
            accountant: Accounting mechanism. Currently supported:
                - rdp (:class:`~customopacus.accountants.RDPAccountant`)
                - gdp (:class:`~customopacus.accountants.GaussianAccountant`)
                - prv (:class`~customopacus.accountants.PRVAccountant`)
            secure_mode: Set to ``True`` if cryptographically strong DP guarantee is
                ...
                See :meth:`~customopacus.optimizers.optimizer._generate_noise` for details.
    ...
    def get_compatible_module(cls, module: nn.Module) -> nn.Module:
        ...
        Returns:
            ...
            close equivalents.
            See :class:`~customopacus.validators.module_validator.ModuleValidator` for
            more details
        """
    ...
    def make_private(
        ...
        **kwargs,
        ...
        Args:
            Vk: new parameter, Gradient projection matrix
            ...
            grad_sample_mode: mode for computing per sample gradients. Determines the
                ...
                :class:`~customopacus.grad_sample.gsm_base.AbstractGradSampleModule` for more
                details
    ...
    def make_private_with_epsilon(
        ...
        **kwargs,
        ...
        """
        Version of :meth:`~customopacus.privacy_engine.PrivacyEngine.make_private`,
        that calculates privacy parameters based on a given privacy budget.

        For the full documentation see
        :meth:`~customopacus.privacy_engine.PrivacyEngine.make_private`
